...
\begin{proof}
	...
	Pick some $a = a_i$.
	Since $\Spec A$ is quasi-compact and $Z \subseteq \Spec A$ is closed, it, too, is quasi-compact and can therefore be covered with finitely many open affine subschemes $Z = \bicup_{j = 1}^n \Spec B_i$.
...
\begin{definition}
	...
	When $S = \Spec k$ for $k$ a field, we will simply speak of $k$-schemes.
\end{definition}
...
Recalling that $\CRing$ has pushouts, given by the relative tensor product $A \tensor_B C$, we obtain that $\AffSch$ has pullbacks via remark \ref{prp:sch:ringaff}.
...
\begin{proof}
	TODO.
\end{proof}
As a consequence, the inclusion $\AffSch \incl \Sch$ preserves limits, in particular pullbacks, so the fibre product of $\Spec A \leftarrow \Spec B \rightarrow \Spec C$ in $\Sch$ is given by $\Spec A \tensor_B C$ together with the obvious structure maps.

Before moving on to the general proof, let us discuss some applications.
\begin{definition}

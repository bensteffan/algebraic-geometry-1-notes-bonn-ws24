...
\begin{definition}
	...
	where 
	\begin{equation*}
		(f)_0 \coloneq \sum_{\substack{Y \in \PrDiv(X) \\ v_Y(f) > 0}} v_Y(f) Y
	\end{equation*}
	is the \defineterm[divisor!zero]{zero divisor} of $f$ and 
	\begin{equation*}
		(f)_\infty \coloneq \sum_{\substack{Y \in \PrDiv(X) \\ v_Y(f) < 0}} v_Y(f) Y
	\end{equation*}
...
\end{equation*}
\begin{definition}
	A \defineterm[divisor!Cartier]{Cartier divisor} on $X$ is an element of $\Gamma(X, \cK_X^\mlt / \cO_X^\mlt)$.
\end{definition}
...
\begin{equation*}
	0 \to H^0(X, \cF) \to H^0(X, \cG) \to H^0(X, \cH) \xto{\delta} H^1(X, \cF) \to \cdots
\end{equation*}
TODO
